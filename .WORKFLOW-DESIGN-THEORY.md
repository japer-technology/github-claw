...
- `.GITCLAW/install/GITCLAW-WORKFLOW-AGENT.yml` — template used during installation
- `.GITCLAW/GITCLAW-INSTALLER.yml` — master copy of the installer
...
1. **A user opens an issue or posts a comment** → the workflow starts.
2. **Authorization gate** — The actor's repository permission is verified via the GitHub API. Only collaborators with `write`, `maintain`, or `admin` permission are allowed through. Unauthorized users cause the workflow to fail immediately, preventing public abuse on open repositories.
3. **Fail-closed sentinel guard** (`GITCLAW-ENABLED.ts`) — Checks for the existence of `.GITCLAW/GITCLAW-ENABLED.md`. If the file is absent, the entire workflow halts. This means the agent is disabled by default on any fresh clone until the operator deliberately opts in by ensuring the sentinel file is present.
4. **Heart guard** (`GITCLAW-HEART-GUARD.ts`) — An optional gate that, when activated (by renaming `GITCLAW-HEART-NOT-REQUIRED.md` to `GITCLAW-HEART-REQUIRED.md`), requires new issues to contain a ❤️ emoji in their body. This only applies to `issues.opened` events; follow-up comments are always processed. When disabled (the default), all issues pass through.
5. **Activity indicator** (`GITCLAW-INDICATOR.ts`) — Adds a 👀 reaction to the triggering issue or comment, providing immediate visual feedback that the agent is working. The reaction metadata is persisted to `/tmp/reaction-state.json` for cleanup later.
6. **Dependency installation** — Runs `bun install --frozen-lockfile` inside `.GITCLAW/` to install the `pi` agent binary and its dependencies.
7. **Agent execution** (`GITCLAW-AGENT.ts`) — The core orchestrator that:
   - Fetches the full issue title and body from the GitHub API.
   - Resolves or creates a session mapping (`state/issues/<number>.json` → `state/sessions/<timestamp>.jsonl`) to maintain conversation continuity across workflow runs.
   ...
   - In a `finally` block, removes the 👀 reaction to signal completion regardless of success or failure.
...
- **Git-native persistence**: All conversation history and state changes are committed to the repository. There is no external database — the git log is the audit trail.
...
1. **Manual dispatch** — An operator clicks "Run workflow" in the Actions tab, optionally configuring overwrite behavior and Node.js version.
2. **Self-modification** — A push that changes the installer workflow file itself triggers a re-run, ensuring the bootstrap stays current.
3. **Post-agent sync** — After every completed run of GITCLAW-WORKFLOW-AGENT, the installer runs to ensure any agent-generated workflow changes are properly installed. A guard (`github.actor != 'github-actions[bot]'`) prevents the installer from triggering on its own commits.
...
1. **Branch creation** — Creates a timestamped branch (`gitclaw/bootstrap-YYYYMMDD-HHMMSS`) to isolate bootstrap changes from the main branch.
2. **Workflow installation with runtime conversion** — Copies all `.yml`/`.yaml` files from `.GITCLAW/install/` to `.github/workflows/`, performing an automatic Bun-to-Node/npm conversion:
   - `oven-sh/setup-bun` → `actions/setup-node@v4`
   - `bun-version` → `node-version`
   - `bun install --frozen-lockfile` → `npm ci`
   - `bun <file>.ts` → `npx tsx <file>.ts`
   - `bun <file>.js` → `node <file>.js`
   - `bun test` → `npm test`
   - `bun run` → `npm run`
...
3. **`.gitignore` management** — Merges ignore rules from `.GITCLAW/.gitignore` or `.GITCLAW/gitignore` into the repository's `.gitignore`, wrapped in marker comments (`# --- GITCLAW managed rules ---`) for idempotent updates. Previous managed blocks are removed before re-adding.
4. **Issue template installation** — Copies files matching `GITCLAW-TEMPLATE-*` from `.GITCLAW/install/` to `.github/ISSUE_TEMPLATE/`, providing standardized issue forms (e.g., the Hatch template for personality bootstrapping).
5. **Config file installation** — Copies optional configuration files (`.editorconfig`, `.nvmrc`, `.npmrc`, `.eslintrc*`, `.prettierrc*`) from `.GITCLAW/` to the repository root, and executable scripts from `.GITCLAW/scripts/` to `scripts/`.
6. **Installation marker** — Renames `.GITCLAW/GITCLAW-NOT-INSTALLED.md` to `.GITCLAW/GITCLAW-INSTALLED.md` to record that the bootstrap has completed.
7. **Pull request creation** — If any changes were made, commits them and opens a pull request against the default branch with a descriptive body listing all changes. This ensures bootstrap changes are reviewed before merging.
...
```
                ...
                │                             │
                │  1. Authorize actor          │
                │  2. Guard (sentinel file)    │
                │  3. Heart guard (optional)   │
                │  4. Add 👀 indicator         │
                │  5. Install dependencies     │
                │  6. Run AI agent             │
                │  7. Commit state + push      │
                │  8. Post reply comment       │
                │  9. Remove 👀 indicator      │
                └─────────┬───────────────────┘
                          │ (on completed)
                          ▼
                ...
                │  GITCLAW-INSTALLER          │
                │                             │
                │  1. Create bootstrap branch  │
                │  2. Install workflows (bun→npm) │
                │  3. Merge .gitignore rules   │
...
The installer's `github.actor != 'github-actions[bot]'` guard prevents this chain from becoming an infinite loop — the installer's own commits do not re-trigger itself.
...
| 3 | `GITCLAW-INDICATOR.ts` | Add 👀 reaction indicator | Non-fatal; agent still runs |
| 4 | `GITCLAW-AGENT.ts` | Run the AI agent and post reply | Agent error; 👀 still cleaned up via `finally` |
...
1. **Actor authorization** — The first step in the agent workflow checks the triggering user's repository permission via the GitHub API. Only `write`, `maintain`, and `admin` collaborators proceed.
2. **Fail-closed sentinel** — The `GITCLAW-ENABLED.md` file must exist for the agent to run. Deleting this file instantly disables all agent automation without removing any code.
3. **Bot loop prevention** — Both workflows filter out actions from `github-actions[bot]` to prevent infinite trigger loops.
4. **Heart gate** — An optional additional layer requiring a deliberate emoji in the issue body, useful for rate-limiting or confirming intent.
5. **Scoped permissions** — Each workflow requests only the minimum GitHub token permissions needed for its operations.
6. **Non-destructive installation** — The installer never overwrites existing files by default, and delivers changes via PR for human review.
...
.GITCLAW/state/
  issues/
    <number>.json           # Maps issue number → session file path
  sessions/
